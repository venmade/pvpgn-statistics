...
class pvpgn_stats {
    ...
    function get_perc($user) {
        ...
        else if (($user[$temp_game . 'losses'] + $user[$temp_game . 'wins']) != 0)
        ...
        if ($game == "WAR3_" || $game == "W3XP_") {
            ...
            if (($user[$game . 'nightelves_wins'] + $user[$game . 'nightelves_losses']) != 0) {
                ...
                $temp[$game . 'nightelves_perc'] = "0.00";
    ...
    function build_other() {
        ...
        for ($n = 0; $n < count($this->stats); $n++) {
            ...
            //bgcolors
            if ($n % 2 == 0)
                $this->stats[$n][$game . 'bgcolor'] = '#271A13';
            else
                $this->stats[$n][$game . 'bgcolor'] = '#111111';
            //Diablo 1 games
            if ($game == "DRTL_") {
                ...
                if (Empty($this->stats[$n]['DRTL_0_class'])) {
                    ...
                    $this->stats[$n]['DRTL_0_image'] = $this->stats[$n]['DRTL_0_class'] . '.gif';
            }
            ...
            if (($game == "STAR_") || ($game == "SEXP_") || ($game == "W2BN_")) {
                ...
                if (($type == '0') or ( $type == '1')) {
                    ...
                    die('Incorrect game type for StarCraft or WarCraft 2 games.<BR>Use 1 for ladder or 0 for normal games!');
            }
            ...
            if (($game == "WAR3_") || ($game == "W3XP_")) {
                ...
                if (($type == 'solo') || ($type == 'team') || ($type == 'ffa')) {
                    ...
                    die('Incorrect game type.<br>Please use team, solo or ffa!');
            }
        }
    }
    ...
    function d2ladder_update() {
        ...
        $query = "SELECT d2ladder_time FROM `$db_counters`";
        $result = $this->data->db_query($query);
        ...
        if ($d2update_time != 0) {
            if (($now >= ($last_update['d2ladder_time'] + $d2update_time)) || ($last_update['d2ladder_time'] == 0)) {
                $update = true;
                $query = "UPDATE `$db_counters` SET d2ladder_time = $now LIMIT 1";
                $result = $this->data->db_query($query);
            } else
                $update = false;
        } else
        ...
        if ($update) {
            ...
            for ($n = 0; $size > 0; $n++) {
                ...
                if ($this->data->sql_num_rows($result) == 0) {
                    $query = "INSERT INTO `$db_d2` (`charname`,`title`,`level`,`class`,`experience`,`type`,`dead`,`game`) VALUES ('$charname','$prefix','$level','$classes[$class]','$xp','$hc','$dead','$game')";
                    $result = $this->data->db_query($query);
                } else {
                    if ($row['experience'] < $xp) {
                        $query = "UPDATE `$db_d2` SET experience=$xp, title='$prefix', level=$level, class='" . $classes[$class] . "', type='$hc', dead='$dead', game='$game' WHERE charname='$charname'";
                        $result = $this->data->db_query($query);
                        if ($result != 1) {
    ...
    function load_d2stats($game, $sort_by, $sort_direction, $start, $stop) {
        ...
        for ($n = 0; $n < count($this->stats); $n++) {
            if ($n % 2 == 0)
                $this->stats[$n][$game . '_bgcolor'] = '#271A13';
            else
                $this->stats[$n][$game . '_bgcolor'] = '#111111';
            global $type;
            //if ($type=='SC') {
